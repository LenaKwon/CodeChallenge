...
function getRating(watchList) {
  // Only change code below this line
  let num = (watchList.filter(movie => movie['Director']==="Christopher Nolan")).length;
  let averageRating=
  watchList.filter(movie => movie['Director']==="Christopher Nolan").reduce((sum,movie)=> (sum + Number(movie.imdbRating)),0)
