...
```
## Thinking 
1. At the beginning, our array videoWindow looks like this:</br>
...
Arrays now look like this:<br>
`tabsBeforeIndex: ['Netflix', 'YouTube']`<br>
`videoWindow: ['Vimeo', 'Vine']`<br>
You can already see __why splice() can be very hazardous__, as it **always modifies the array** it is executed upon and returns that modified array - it is not deterministic.

4. After second line is executed, `tabsAfterIndex` should take values starting from index (in this case 2) and remove them from `videoWindow` array. <br>
As we can see that in the current state (`['Vimeo', 'Vine']`), `videoWindow` does *not* have index `2` so *empty array* is returned. Final state:<br>
`tabsBeforeIndex: ['Netflix', 'YouTube']`<br>
`videoWindow: ['Vimeo', 'Vine']`<br>
`tabsAfterIndex: []`<br>
